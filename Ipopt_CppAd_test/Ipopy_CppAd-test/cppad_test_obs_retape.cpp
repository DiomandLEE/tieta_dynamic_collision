...
class FG_eval
{
	public:
	    // constructor
        AD<double> pow_(AD<double> x, AD<double> y)
        {
            return (CppAD::pow(x - 2, 2) + CppAD::pow(y - 0, 2));
        }
	    ...
	    void operator()(ADvector& fg, const ADvector& x)
	    {
            ...
            fg[0] += 10 * CppAD::pow(x4 - 3, 2);
            fg[0] += 10 * CppAD::pow(x5 - 4, 2);
            ...
            if ((pow_(x1,y1)) < 2.25)
            {
                fg[0] += 1 * CppAD::pow(CppAD::sqrt(pow_(x1,y1)) - 1.5, 2);
            }
            if ((pow_(x2,y2)) < 2.25)
            {
                fg[0] += 1 * CppAD::pow(CppAD::sqrt(pow_(x2,y2)) - 1.5, 2);
            }
            if ((pow_(x3,y3)) < 2.25)
            {
                fg[0] += 500 * CppAD::pow(CppAD::sqrt(pow_(x3,y3)) - 1.5, 2);
            }
            if ((pow_(x4,y4)) < 2.25)
            {
                fg[0] += 500 * CppAD::pow(CppAD::sqrt(pow_(x4,y4)) - 1.5, 2);
            }
            if ((pow_(x5,y5)) < 2.25)
            {
                fg[0] += 1 * CppAD::pow(CppAD::sqrt(pow_(x5,y5)) - 1.5, 2);
            }
