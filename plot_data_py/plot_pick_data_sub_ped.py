...
ax1.plot(time_column, [1.25] * len(time_column), '--', color='green', label='Soft Constraint',linewidth=2.5)
...
ax1.legend(loc="upper right",bbox_to_anchor=(.85, 1.0))
ax1.grid(True)
...
ax2.plot(time_column, distance_csv.iloc[:, 4], label='Gripper', color=colors[4])
...
# 添加碰撞线
ax2.plot(time_collision, [0.32] * len(time_collision), 'o-', color='red', label='Collision',markerfacecolor='none', linewidth=2.5)
#ax2.axhline(y=0.06, color='black', linestyle='--')
...
ax2.legend(loc="upper right",bbox_to_anchor=(.85, 1.0))
ax2.grid(True)
