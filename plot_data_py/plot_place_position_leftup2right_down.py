...
plt.title('Place Action Positions Over Time',fontweight='bold')
...
for i in range(len(list_new_vels)):
    ...
    plt.plot(x_long,df_position[0], color=colors[i], label=labels[i],linewidth=2.5)


# # 输出结果
# print(df)
# # 根据速度数据计算位移
# df_displace = list_new_vels[0] * new_timestep
...
# # 在 t=0.2s 时的位置为 10 米
# position_at_39 = csv_pos[0]

# # 根据 t=0.2s 时的位置来调整初始位置
# initial_position = position_at_39 - df_position[0].iloc[39]
# # print("..",df_position[0].iloc[39])
